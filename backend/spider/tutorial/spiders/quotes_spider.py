    ...
    def parse(self, response):
        ...
        if self.url.find('revolve')!= -1:
            ...
            if not response.css('div.container').css('main#main-content').css('div.g').css('div.gc').css('div.prices').css('span.prices__retail'):
                print 'markdown'
                current_price = json.loads(response.css('div.container').css('main#main-content').css('div.g').css('div.gc').css('div.price').css('div.price__markdown').xpath('text()').extract()[0].replace(',','').strip('$'))
            else:
                print 'normal price'
                current_price = json.loads(response.css('div.container').css('main#main-content').css('div.g').css('div.gc').css('div.price').css('span.price__retail').xpath('text()').extract()[0].replace(',','').strip('$'))
